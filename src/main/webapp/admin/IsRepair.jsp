...
<script>
    ...
    $(document).ready(function() {
        ...
        $.ajax({
            type:'post',
            url:'/admin/repair/getRepairList.do?page=1&repairStatus=1',
            async: false,
            success:function (data) {
                ...
                $.each(data.data, function(i,item){
                    $("#mainTable").append('<tr role="row" class="odd">' +
                            ...
                            '<td class="sorting_1">'+new Date((item.resolveTime)).Format("yyyy-MM-dd hh:mm:ss")+'</td>' +
                            ...
                            '<td class="sorting_1"><a href="/admin/repair/readRepair.do?repairNum='+item.repairNum+'">查看</a></td></tr>')
                });
    ...
    function getTable(now) {
        ...
        $.ajax({
            type: 'post',
            url: '/admin/repair/getRepairList.do?repairStatus=1&page=' + now,
            async: false,
            success: function (data) {
                totalNum = data.totalNum;
                currentPage = data.pageNum;
        ...
        var option = {
            onOk: function () {
                ...
                if(position == ''){
                    ...
                    var option = {
                        onOk: function () {
                            window.location.href = "/admin/IsRepair.jsp";
                        }
                    };
                ...
                $.ajax({
                    type: 'post',
                    url: '/admin/repair/deleteRepair.do?position=' + position,
                    async: false,
                    success: function (data) {
                        if (data.code == 1) {
                            var txt = "删除成功！";
    ...
    function searchRepair(page) {
        $("#refleshRepair").show();
        $("#mainTable").empty();
        var repairSearch=$("#repairSearch").val();
        $.ajax({
            type: 'post',
            url: '/admin/repair/repairSearch.do?repairStatus=1&page='+page+'&repairSearch='+repairSearch,
            async: false,
            success: function (data) {
                totalNum = data.totalNum;
                currentPage = data.pageNum;
                ...
                $.each(data.data, function (i, item) {
                    $("#mainTable").append('<tr role="row" class="odd">' +
                            ...
                            '<td class="sorting_1">'+item.addrezz+'</td>' +
                            '<td class="sorting_1">'+item.studentNum+'</td>' +
                            '<td class="sorting_1"><a href="/admin/repair/readRepair.do?repairNum='+item.repairNum+'">修改</a></td></tr>')
                });
                $("#list").empty();
                $("#list").append('<li class="paginate_button previous" id="example1_previous"><a id="pre"  href="#" aria-controls="example1" data-dt-idx="0" tabindex="0">上一页</a></li>');
                $('#pre').attr('href','JavaScript:searchRepair('+(pre)+');');
                for (var i = 0; i < pageAmount; i++) {
                    $("#list").append('<li class="paginate_button" id="example1_current'+(i+1)+'"><a id="current'+(i+1)+'" href="#" aria-controls="example1" data-dt-idx="1" tabindex="0">'+(i+1)+'</a></li>');
                    $('#current'+(i+1)).attr('href','JavaScript:searchRepair('+(i+1)+');');
                    if ((i+1)==currentPage) {
                        $("#example1_current"+(i+1)).addClass('active');
                    }
                }
                $("#list").append('<li class="paginate_button next" id="example1_next"><a id="next" href="#" aria-controls="example1" data-dt-idx="7" tabindex="0">下一页</a></li>');
                $('#next').attr('href','JavaScript:searchRepair('+(next)+');');
                if (pageAmount==1){
                    $("#example1_previous").addClass('disabled');
                    $('#pre').attr('href','#');
                    $("#example1_next").addClass('disabled');
